...
whole canon is 48,068 verses; most of them are narrative, law, genealogy or
...
node scripts/check_render.js             # render all 48,068 verses, fail on anything visible
...
A second, stricter pass — `scripts/audit_structure.py`, which demands every
chapter run 1..N and every verse number follow its neighbour — then found
what the word-level scan could not, because the missing words also stand
elsewhere in the canon or were never printed at all:

| Where | What had gone |
|---|---|
| 1 Esdras 4:52-57, 2 Esdras 4:52, 15:52, 1 Maccabees 6:55, 9:8, 9:18, 10:88, 11:1, 11:36, 2 Maccabees 8:1, 13:8, 13:16, 13:18, 14:1 | verses whose bracketed numbers the scan set as letters or ate a bracket — `[S2]`, `[lS]`, `[551`, `136]`, `[lJ` — recovered by trusting a repaired reading only when it lands on exactly the next verse |
| Sirach 12:8, 16:1-6, 11:28 and more | the same fault across the Wisdom of Sirach |
| Shepherd of Hermas 16:9, 76:2, 105:6-8, 106:1, 107:1 | markers garbled to "8116:9", "27[1 04]:1" |
| Yoḇelim 2:5-8, 13:11, 14:21, 14:23, 16:31, 19:15, 46:1 | verse numbers welded to their first word ("21Abram rejoiced") or set as letters ("S On the third day") |
| Ḥanoḵ (Ethiopic) 12:5-8, 16:3, 20:4-8, 69:22 | page numbers landing mid-verse and bracketed restorations the marker rules refused |
| Luke 21:2, Yirmeyahu 51:2 | the volume never printed the "2"; the words sat inside verse 1 |
| Yaḥezqĕl 37:18 | printed pages out of order — its words sit between 37:6 and 37:7 in the page flow |
| Mishlĕ 12:13 | its marker clipped to "3" at the column edge |
| Ḥanoḵ (Jay Winter) 9:31 | marker set as "3l" |
| Miḵah 7:17, Iyoḇ 24:21, and verse 1 of the Prayer of Azariah, Prayer of Menashsheh, Tehillah 151 and Shoshannah | the printings omit these verses outright; they are supplied in the edition's own voice and carry a footnote saying so |

The page-number scars are gone at the root: every extractor that reads the
Lumpkin volume now removes the printed folio beside the running banner — and
only that page's own number, because a verse marker often opens the line
right after the banner. Lumpkin's own commentary ("[Author's note: …]") is
likewise removed from the reading: it is apparatus, and its inner numbers
were spawning phantom verses (Ḥanoḵ 84 grew a "verse 10" out of "See Daniyal
Chapter 10").

A third pass then went after the vouched list itself, and closed every gap
whose words could honestly be recovered. Seven turned out to be merges the
scan had hidden — the "missing" verse's words were sitting inside the verse
before it, behind a marker the scan had garbled ("67" left inline in Yashar
23, "n" standing for 11 in Yoḇelim 32 and 41, "S" for 8 in Yoḇelim 48) —
and were split back out at the print's own marker: Yashar 23:67 and 63:24,
Yoḇelim 19:25, 19:26, 32:11, 41:11 and 48:8. One more, Sirach 7:3, survived
in the scan only as garbage bytes ending "…a sevenfold crop" and was
restored with the RSV wording the volume is set from, marked with a
footnote. The 66 canonical books were then compared, chapter for chapter
and verse for verse, against the standard versification
(`scripts/verify_versification.py`): **all 66 match exactly** — the one
difference it found was Miḵah 7:20 duplicated into the end of Naḥum 3,
which is now removed.

What remains is `scripts/audit_structure.py`'s vouched list: 26 numbering
gaps that mirror the printed pages themselves — and behind the pages, the
RSV translation those books are set from. The RSV-lineage apocrypha omit
those verses as unattested (most of Sirach's, 4 Maccabees 10:4 and 11:7-8 —
the RSV itself runs 11:6 straight to 11:9), or group them into ranges
("[15-26]"), and Additions to Esther opens at verse 2 because the print
sets the Greek colophon at the book's end (7:11). Each was checked against
its page before being listed, and the audit fails on any gap that is not on
that list. Filling them would mean splicing a different translation into
the reading, so they stand as the edition prints them.
...
a search of all 48,068 verses and why each survivor is correct.
...
```bash
pip install pypdf pdfplumber english-words pythonbible
...
python3 scripts/restore_source_defects.py  # re-applies the repairs the PRINTS need
                                       #   (verses their plates merged, misplaced
                                       #   or omitted; see below)
python3 scripts/find_missing_text.py   # nothing printed in the PDFs was left behind
python3 scripts/audit_structure.py     # every chapter 1..N, every verse contiguous
python3 scripts/verify_versification.py    # all 66 canonical books match the
                                       #   standard versification exactly

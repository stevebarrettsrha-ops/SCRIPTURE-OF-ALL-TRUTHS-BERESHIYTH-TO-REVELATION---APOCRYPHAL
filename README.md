...
whole canon is 48,061 verses; most of them are narrative, law, genealogy or
...
node scripts/check_render.js             # render all 48,061 verses, fail on anything visible
...
What remains is `scripts/audit_structure.py`'s vouched list: 33 numbering
gaps that mirror the printed pages themselves — verses the RSV-lineage
apocrypha omit (most of Sirach's), group into ranges ("[15-26]"), or that
the 1887 Yashar plates skip — each checked against its page before being
listed. The audit fails on any gap that is not on that list.
...
a search of all 48,061 verses and why each survivor is correct.
